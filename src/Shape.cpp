...
Shape::Shape(const Shape& other) {
    ...
    // Create new OpenGL buffers
    glGenVertexArrays(1, &vao);
    glGenBuffers(1, &vbo);
    
    // Calculate total vertices
    size_t totalVertices = pos.size();
    
    // Prepare buffer data
    std::vector<float> vertexData;
    vertexData.reserve(totalVertices * 6); // 3 floats per position + 3 floats per normal
    ...
    for (size_t i = 0; i < norm.size(); i++) {
        vertexData.push_back(norm[i].x);
        vertexData.push_back(norm[i].y);
        vertexData.push_back(norm[i].z);
    }
